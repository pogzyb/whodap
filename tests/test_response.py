...
class TestDomainResponse(unittest.TestCase):
    ...
    def test_to_whois_dict(self):
        ...
        for k, v in whois_dict.items():
            ...
            if k == WHOISKeys.CREATED_DATE or k == WHOISKeys.UPDATED_DATE or k == WHOISKeys.EXPIRES_DATE:
    ...
    def test_to_whois_json(self):
        ...
        for k, v in json.loads(whois_json).items():
            assert k in whois_dict.keys(), f"key {k} from whois_json is not in whois_dict"

    def test_convert_self_to_dict(self):
        json_string = '{"list_key1": [{"inner_dict1": 1}, {"inner_dict2": 2}], "dict_key1": 1}'
        ...
        self.assertEqual(list(resp_dict.values()), list(json.loads(json_string).values()))
    ...
    def test_convert_dates(self):
        ...
        for ds in (ds_1, ds_2, ds_3):
            assert type(self.resp._convert_date(ds)) == datetime, f"failed to convert to datetime"
        ds_4 = "unsupported"
        assert type(self.resp._convert_date(ds_4)) == str, f"unexpected output from _convert_date"
    ...
    def test_flat_entities(self):
        ...
        for role, role_values in flattened_entities.items():
            assert role in ('abuse', 'registrant', 'registrar', 'technical', 'billing', 'administrative'), f'{role} not found'
            for key, value in role_values.items():
                assert key in ('org', 'abuse', 'address', 'phone', 'name', 'email', 'fax'), f'role_value key {key} not found'
                assert value is not None

    def test_construct_flat_dict(self):
        flat = {'administrative': {'address': self.address, 'phone': self.phone, 'name': 'test'}}
        flat_converted = self.resp._construct_flat_dict(flat)
        num_flat_keys = len(flat_converted.keys())
        num_whois_keys = len([k for k in dir(WHOISKeys) if not k.startswith('__')])
        assert flat_converted['admin_address'] == self.address, f"{flat_converted['admin_address']} != {self.address}"
        assert flat_converted['admin_phone'] == self.phone, f"{flat_converted['admin_phone']} != {self.phone}"
        assert flat_converted['admin_name'] == 'test', f"{flat_converted['admin_name']} != test"
        assert num_flat_keys == num_whois_keys - 2, f'{num_flat_keys} != {num_whois_keys}'
